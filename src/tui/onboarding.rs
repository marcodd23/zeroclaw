...
fn apply_tui_selections_to_config(app: &App, config: &mut Config) {
    ...
    if search != "Skip for now" && search != "None" {
        let search_id = match search {
            ...
            // Tavily and Google CSE are not yet supported by the config
            // schema. Map them to duckduckgo as a safe fallback and log
            // the user's intent so they can reconfigure later.
            "Tavily" | "Google Custom Search" => "duckduckgo",
            _ => "duckduckgo",
        };
