...
//! An optional HMAC auth token (CHELAR_SANDBOX_AUTH_TOKEN) is sent as a Bearer
//! header for request authentication.
...
pub struct CodeExecTool {
    ...
    /// Full URL for the sandbox exec endpoint (e.g. "http://172.16.0.1:8081/api/sandbox/exec").
    api_url: String,
    /// Tenant ID for the sandbox request.
    tenant_id: String,
    /// Per-tenant HMAC auth token for sandbox requests (CHELAR_SANDBOX_AUTH_TOKEN).
...
impl CodeExecTool {
    ...
    pub fn from_env(security: Arc<SecurityPolicy>) -> Option<Self> {
        ...
        let auth_token = std::env::var("CHELAR_SANDBOX_AUTH_TOKEN")
            .ok()
            .filter(|t| !t.is_empty());
