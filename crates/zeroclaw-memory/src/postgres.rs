//! PostgreSQL-backed persistent memory for Chelar multi-tenant deployments.
//!
//! This backend is used instead of SQLite when tenant containers run on JuiceFS
//! (FUSE), where SQLite WAL mode is unreliable. Memory is stored in a shared
//! Supabase PostgreSQL instance.
//!
//! Focus: reliable CRUD and keyword recall using full-text search (tsvector),
//! without requiring extension setup (e.g. pgvector).
...
impl PostgresMemory {
    /// Create a new PostgresMemory backend.
    ///
    /// `schema` and `table` must be valid SQL identifiers (letters, digits,
    /// underscores only). They are quoted to prevent injection.
    pub fn new(
        db_url: &str,
        schema: &str,
        table: &str,
    ...
    /// Spawn a plain OS thread to initialize the postgres connection and schema.
    /// The sync `postgres` crate calls `Runtime::block_on()` internally, which
    /// conflicts with tokio's blocking pool threads. Plain OS threads have no
...
/// panics. See `initialize_client` for rationale.
